...
pas=5
fichier_graph='graph_p'+str(pas)+'_jour.csv'
fichier_comp='comp_p'+str(pas)+'_jour.csv'
...
minute_2019_debut=6*24*60-3*60
minute_2019_fin=7*24*60+3*60
...
while a<51*7:
    ...
    graphcsv.writerow(x[1])
    graphcsv.writerow(x[0])
    comp=np.array(comparaison_numpy(x[0],x[1]))
    comp=comp*comp
    max_comp=max(comp)
